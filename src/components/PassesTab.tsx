           ...
           <div key={i} className={`relative border rounded-[6px] p-5 sm:p-6 ${p.featured ? 'border-2 border-[#3a3a37] bg-[#fbfbf9]' : 'border-[#e1ded7] bg-[#fdfdfb] hover:border-[#cdc9c1] transition-colors'}`}>
             ...
             <div className="flex justify-between items-start mb-4 sm:mb-5 gap-3">
               <div className="flex-1">
                 ...
                 <div className="text-[13px] sm:text-xs text-[#807c73] mt-1 sm:mt-1.5 leading-relaxed">{p.desc}</div>
               </div>
               <div className="text-right shrink-0">
                 <div className={`font-mono text-[18px] sm:text-[17px] font-medium ${p.price === 'TBD' ? 'text-[#b3afa6]' : 'text-[#33312d]'}`}>{p.price}</div>
                 <div className="font-mono text-[9px] text-[#a8a49b] mt-1">{p.sub}</div>
               </div>
             </div>
             <a href={p.link} target="_blank" rel="noopener noreferrer" className={`w-full cursor-pointer h-10 sm:h-[38px] rounded flex items-center justify-center gap-1.5 font-mono text-[12px] sm:text-[11px] no-underline transition-colors ${p.featured ? 'bg-[#3a3a37] text-[#e9e7e1] hover:bg-[#4a4a47]' : 'border border-[#cdc9c1] text-[#807c73] hover:bg-[#f4f2ec]'}`}>
               select on CivicRec
